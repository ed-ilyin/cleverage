...
let колонка ``class`` =
    span [ join " " [ "column"; ``class`` ] |> attr.``class`` ]
...
let игрок и =
    div [ attr.``class`` "list-item columns" ] [
        колонка "is-narrow symbol" [ text и.Символ ]
        колонка "" [
            div [ attr.``class`` "columns" ] [
                колонка "" [ text и.Имя ]
                колонка "is-narrow" [ sprintf "😱 %i" и.Подозрительность |> text ]
            ]
            плохость и
            // List.map (text >> List.singleton >> li []) и.Подозрительно |> ol []
        ]
        колонка "is-narrow" [
            d "columns" [ роль "🧘" и.Спаситель; роль "🕺" и.Масон ]
            d "columns" [ роль "🦖" и.Рептилоид; роль "😈" и.Демон ]
        ]
    ]
